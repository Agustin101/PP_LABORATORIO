...
int agregarCensista(Censista *list, int len) {
	...
	if (list != NULL && len > 0 ) {
		...
		if (indiceVacio != -1){
			if(getString(aux.name,"Ingrese el nombre del censista:\n","Por favor ingrese un nombre valido conformado de letras y espacios de ser necesarios.\n",2)==0 && getString(aux.lastName,"Ingrese el apellido del censista:\n","Por favor ingrese un nombre valido conformado de letras y espacios de ser necesarios.\n",2)==0 && utn_getInt(&aux.datosNacimiento.anioNacimiento, "Indique el año de nacimiento del censista", "Error", 1960,2002,2)== 0 &&
					utn_getInt(&aux.datosNacimiento.diaNacimiento, "Indique el dia de nacimiento del censista (1 al 31)", "Error", 1,31,2)== 0 && utn_getInt(&aux.datosNacimiento.mesNacimiento, "Indique el mes de nacimiento del censista (1 al 12)", "Error", 1,12,2)== 0 && getString(aux.direccionCensista.calle,"Ingrese la calle donde vive el censador:\n","Por favor ingrese un nombre valido conformado de letras y espacios de ser necesarios.\n",2) == 0 && utn_getInt(&aux.direccionCensista.altura, "Indique la altura.", "Error", 1,20000,2)== 0 &&
					utn_getInt(&aux.edadCensista, "Indique la edad del censista: \n", "Error", 18,65,2)==0){
				list[indiceVacio] = aux;
				list[indiceVacio].estadoCensista = LIBERADO;
				list[indiceVacio].idCensista = censistaId();
 ...
 */
/*
int sortPassengersByFlyCode(Censista *list, int len, int order) {
	int retorno;
	int i;
	int estaOrdenado;
	retorno = -1;
...
int modificarCensista(Censista *list, int len, int id){
	...
	if (list != NULL && len > 0 && id > 0) {
		indiceAModificar = findCensistaById(list, len, id);
